...
namespace StarSea.office
{
    public class WriteData
    {
        ...
        //向excel写一个单行数据
        public void writeSingeRow( Starsea.office.ExcelTools et,string sheetId, string sql, string write, string[,] parms)
        {
            ...
            for (int i = 0; i < row; i++)
            {
              ...
              //  Microsoft.Office.Interop.Excel.Range range1 = wst.getRange(worksheet, Convert.ToInt32(ranges[i, 0]), Convert.ToInt32(ranges[i, 1]));
                ...
                et.UpdateValueBySheetId(sheetId,ranges[i, 0], dt.Rows[0][ranges[i, 1]].ToString(),0,true);
            }
        ...
        //写一个表格数据
        public void writeTable(Starsea.office.ExcelTools et, string sheetId, string sql, string write, string[,] parms)
        {
                ...
                for (int j = 0; j < row; j++)
                {
                 //   worksheet.Cells[Convert.ToInt32(ranges[j, 0]) + i, Convert.ToInt32(ranges[j, 1])] = dt.Rows[i][ranges[j, 2]].ToString();
                  ...
                  //  wst.setRangeBorderColor(range1, System.Drawing.Color.Black);
                    ...
                    et.UpdateValueBySheetId(sheetId, et.getXY(ranges[j, 0])[0] + (Convert.ToInt32(et.getXY(ranges[j, 0])[1])+i).ToString(), dt.Rows[i][ranges[j, 1]].ToString(), 0, true);
        ...
        public WriteData(Starsea.office.ExcelTools et, ArrayList list, string[,] parms)
        {
